...
argument-hint: '<ドキュメントパス> [mode: composite] [doc_type: 要件定義書|ADR|DesignDoc]'
context: fork
---
...
- @.claude/steering/documentation-criteria.md - ドキュメント作成基準（レビュー品質基準）
- @.claude/steering/technical-spec.md - プロジェクトの技術仕様書
- @.claude/steering/typescript.md - TypeScript開発ルール（コード例の検証に必要）
...
- **doc_type**: ドキュメントタイプ（`要件定義書`/`ADR`/`DesignDoc`）
...
- Design Docの場合は「前提となるADR」セクションに記載されたADR（`specs/adr/`配下）および類似する過去のADRを確認
...
テンプレートの保存場所は @.claude/steering/documentation-criteria.md に準拠。
