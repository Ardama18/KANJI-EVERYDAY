...
- **@.claude/steering/typescript-testing.md** - テスト設計の基準（品質要件、テスト構造）
- **@.claude/steering/documentation-criteria.md** - ドキュメント基準（Design Doc/要件定義書の構造、AC記載形式）
...
- **配置・命名**: typescript-testing.md「命名規則」に準拠
...
### ビジュアル検証チェックリスト（Visual Verification Checklist）
- **目的**: Figmaデザインと実装の視覚的一致を確認
- **スコープ**: UIページ・コンポーネントの視覚検証
- **生成ファイル**: `specs/stories/{STORY_ID}-{title}/ui-design/visual-checklist.md`
- **実行タイミング**: Phase完了時（ページが機能的に動作する状態）
- **生成条件**: UIを含むタスクのみ（Design Docにui_designパスが記載されている場合）
...
2. 該当URLをブラウザで開く
3. スクリーンショット撮影
4. Claude Codeにアップロード
5. 「@visual-checker [ID]-[feature] [ページ名] [スクリーンショットパス]」で実行

## 完了条件
- [ ] 全ページのビジュアル検証完了（一致率90%以上または承認済み）
```
...
```json
{
  ...
  "generatedFiles": {
    ...
    "visualChecklist": "specs/stories/{STORY_ID}-{title}/ui-design/visual-checklist.md (UIタスクのみ)"
  },
  "testCases": {
    "integration": {
      "total": 10,
...
- 配置・命名: typescript-testing.md「命名規則」「ディレクトリ構造」に準拠
- ビジュアルチェックリスト: `.visual-checklist.md`（Markdown形式）
- 出力: it.todoのみ（実装コード除外）

**ファイル操作**:
- 既存ファイル: 末尾追加・重複防止
- 新規作成: 検出構造に準拠
- **重要**: 統合テスト、E2Eテスト、ビジュアルチェックリストは必ず別ファイルとして生成
- **ビジュアルチェックリスト**: `specs/stories/{STORY_ID}-{title}/ui-design/` に生成（UIタスクのみ）
