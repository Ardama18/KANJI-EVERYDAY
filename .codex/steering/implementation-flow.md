...
共通の品質サイクル、サブエージェントの呼び出し規律、安全則、`--until` / `--auto` の意味は `${CODEX_PLUGIN_ROOT}/shared/orchestration-default.md` を正とする。本ファイルは、入力解決、規模別フロー、停止ポイント、テスト方針、成果物、CHECKPOINT を定義する。両者が競合する場合は、フローと記録先は本ファイル、品質・安全規律は共通既定を優先する。
...
5. 対象コードと既存テスト
6. UI 変更時は `.codex/steering/design-system.md`
...
- plugin skill は必ず `$plugin:skill` 形式で呼び出す。
- `$ar-core:implement`、`$ar-core:quality-fixer`、`$ar-frontend:ui-review` のように記述し、旧形式の `/ar-core:implement` や `/ar-frontend:ui-review` は使用しない。
- Codex custom agent は bare name で起動する。例: `Spawn task-executor to implement the complete plan.md.`
- custom agent 名に `# KANJI-EVERYDAY Implementation Flow
...
7. DB・認証変更時は `supabase/migrations/` と関連する Supabase 実装
 ...
 を付けない。`requirement-analyzer`、`technical-designer`、`work-planner`、`task-executor`、`code-reviewer` は skill ではなく agent として扱う。
- 子セッションが必要な場合は `codex exec` を使用し、legacy CLI runner は使用しない。
