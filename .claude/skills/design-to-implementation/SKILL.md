...
argument-hint: <ストーリーディレクトリ> [追加指示]
...
```
/design-to-implementation <ストーリーディレクトリ> [追加指示]
```

**引数**：
- `<ストーリーディレクトリ>` (必須): ストーリーディレクトリパス（例: `specs/stories/S-0001-database-schema-prisma`）
...
✅ PR作成・ステータス更新（meta.jsonのstatus更新）
...
いずれかが欠けている場合：
- エラーメッセージを表示
- `/story-to-design <ストーリーディレクトリ>` の実行を促す
- 処理を中断
...
4. ✅ PRが作成されている
5. ✅ meta.jsonのstatusがimpl_reviewに更新されている
6. ✅ meta.jsonにPR情報（github_pr_url, github_branch）が記録されている
...
4. meta.jsonのstatusを "impl_review" に更新（Editツール使用）

5. meta.jsonのgithub_pr_url, github_branchを更新
...
**目的**: 実装フェーズ完了をmeta.jsonに記録し、ユーザーに報告
...
3. **完了報告**
   ...
   ## 次のステップ
   1. 実装レビューを受ける（meta.jsonのstatusをimpl_reviewに更新済み）
...
- [ ] PRが作成されている
- [ ] meta.jsonのstatusがimpl_reviewに更新されている
...
- 対処: エラーメッセージを表示、`/story-to-design <ストーリーディレクトリ>` の実行を促す
...
```bash
# 例1: ストーリーディレクトリパスで指定
/design-to-implementation specs/stories/S-0001-database-schema-prisma

# 例2: 追加指示付き
/design-to-implementation specs/stories/S-0002-parent-auth-system "パフォーマンス重視で実装"
```
