...
5. ✅ meta.jsonのstatusがdesign_reviewに更新されている
...
**目的**: 設計・計画フェーズ完了をmeta.jsonに記録し、ユーザーに報告

## 実行フロー（詳細）

### Phase 1: 要件分析
1. **ストーリー情報取得**（ストーリーディレクトリ指定時）
   - ローカルの story.md / requirements.md をReadで読み取り
   - meta.jsonを読み取り（story_id、epic_id等）

2. **requirement-analyzer実行**
   - 入力: ストーリー情報（**要求情報を含む**） + ユーザー追加指示
   - 出力: 作業規模判定（大規模/中規模/小規模）、推奨アプローチ
   - **要求情報の活用**: ローカルの詳細な要件をrequirements.mdに反映
...
1. **technical-designer実行**
   - 入力: 要件情報（**requirements.md情報を含む**）、プロジェクトコンテキスト、追加指示
   - 出力: ADR（アーキテクチャ決定記録）、design.md（技術設計書）
   - **要求情報の活用**: ローカルの要件詳細をdesign.mdの機能要件・受入条件に反映
...
### Phase 5: ステータス更新と完了報告
1. **meta.jsonのstatus更新**
   - statusを "design_review" に更新（Editツール使用）
...
2. **完了報告**
   ...
   ## 次のステップ
   1. 設計レビューを受ける（meta.jsonのstatusをdesign_reviewに更新済み）
...
- [ ] meta.jsonのstatusがdesign_reviewに更新されている
...
### ストーリー情報取得失敗
- ストーリーディレクトリが存在しない、またはmeta.json/story.md/requirements.mdが見つからない
...
/design-to-implementation <ストーリーディレクトリ>
```
...
**次のステップ**：
- 設計レビューを受ける
- `/design-to-implementation <ストーリーディレクトリ>` で実装を開始
