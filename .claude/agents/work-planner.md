...
1. **Design DocからFigma UI仕様パスを取得**
   ...
   - 「Figmaデータソース」または画面構造マップの記載からUI仕様パスを特定
   - 例: `specs/stories/DEBT-S-0001-user-registration/ui-design/`

2. **作業計画書のメタデータに記録**
   - メタデータセクションに`ui_design`フィールドを追加
   - 後続のタスク分解で参照されるため必須
...
created: 2025-01-15
based_on: specs/stories/DEBT-S-0001-user-registration/design.md
ui_design: specs/stories/DEBT-S-0001-user-registration/ui-design/
---
```
...
- [ ] 単体テスト作成・実行完了
- [ ] **ui-fixerで全チェックパス**
  - cache: specs/stories/DEBT-S-0001-user-registration/ui-design/
  - component: Button
...
**UI実装を含むがFigma UI仕様が未作成の場合**:
- Design Docに「Figma UI仕様未作成」と記載されている場合
- 作業計画書に「ui_design: none」と明記
...
- ビジュアル検証: `specs/stories/{STORY_ID}-{title}/ui-design/visual-checklist.md` が存在する場合、「ビジュアル検証実行」を最終Phaseに配置

**テストファイルパス（必須）**:
各テストタスクには具体的なテストファイルパスを明記すること。
- 配置・命名規則: @.claude/steering/typescript-testing.md「命名規則」「ディレクトリ構造」に準拠
