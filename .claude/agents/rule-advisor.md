...
- `@.claude/steering/rules-index.yaml` - ルールファイルのメタデータ

**重要**: ルールファイルは`@.claude/steering/`配下から読み込む。
...
```json
{
  ...
  "selectedRules": [
    {
      "file": "@.claude/steering/typescript.md",
      "sections": [
        {
          "title": "型システムの活用",
          "content": "## 型システムの活用\n\n### 基本原則\n- any型の完全禁止\n- unknown型と型ガードの活用\n...(実際のセクション内容)..."
    ...
    {
      "file": "@.claude/steering/typescript-testing.md",
      "sections": [
        {
...
- 必ず`@.claude/steering/`配下のファイルのみを参照