...
0. **metadata.jsonの読み込み【最初に実施】**
   - `specs/stories/{STORY_ID}-{title}/ui-design/metadata.json` を確認
   - **複数画面（sources）の存在を確認**：
     ```json
     {
       "extraction_summary": { "total_urls": 8, ... },
       "sources": [
         { "nodeId": "0:233", "nodeName": "シナリオ詳細画面", "file": "...", "screenshot": "..." },
         { "nodeId": "0:252", "nodeName": "編集状態", ... },
         ...
       ]
     }
     ```
   - **全ての`sources`に対して以下のステップ1-5を実行**
   - 各画面のstructure.xmlは`structures/`フォルダに格納

1. **各画面のstructure.xmlの読み込み**
   - `specs/stories/{STORY_ID}-{title}/ui-design/structures/{nodeId}-{nodeName}-structure.xml` を確認
   - 画面全体の階層構造を把握
...
2. **スクリーンショットで全体構成確認【推奨】**
   - metadata.jsonの`sources`配列から各画面のスクリーンショットパスを取得
   - **スクリーンショットが存在する場合**：
     - `specs/stories/{STORY_ID}-{title}/ui-design/screenshots/` 内のPNGファイルをReadツールで読み込み
     - 全体の視覚的な構成を把握：
...
3. **TSXファイルで詳細仕様確認【必須】**
   - `specs/stories/{STORY_ID}-{title}/ui-design/components/` 内の全TSXファイルを読み込み
   - 各コンポーネントの詳細仕様を抽出：
...
**Figmaデータソース**: `specs/stories/DEBT-S-017-scenario-management/ui-design/metadata.json`
**画面数**: 8画面
...
**構造ファイル**: `structures/0-233-シナリオ詳細画面-structure.xml`
...
| セクション | 配置 | コンポーネント数 | レイアウト |
|----------|------|-----------------|-----------|
| BasicInfoSection | 上部 | 2 | flex-col, gap-4 |
| ActionListSection | 中部 | N+1 | flex-col, gap-2 |

---

#### 画面2: 編集状態 (0:252)

**構造ファイル**: `structures/0-252-編集状態-structure.xml`
...
- [ ] **AC-Structure-1** (遍在型): システムは全8画面をmetadata.jsonのsources順に実装すること
- [ ] **AC-Structure-2** (遍在型): 各画面のコンポーネントはFigmaキャッシュのTSXファイルに準拠すること

### 画面別の受入条件

#### シナリオ詳細画面
- [ ] **AC-Detail-1** (契機型): ユーザーがシナリオを選択したとき、システムはシナリオ詳細画面を表示すること
- [ ] **AC-Detail-2** (遍在型): システムはアクション一覧を実行順序で表示すること

#### 編集状態
- [ ] **AC-Edit-1** (契機型): ユーザーが編集ボタンをクリックしたとき、システムは編集状態に遷移すること
```
