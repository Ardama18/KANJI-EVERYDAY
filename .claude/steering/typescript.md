...
**フォーマット規則**
- セミコロン省略（Biomeの設定に従う）
- 命名規則: `.claude/rules/naming-convention.md` を参照
- インポートは各サブプロジェクト（backend/frontend）のtsconfig.json baseUrl設定に従う
  - backend内: `import { UserService } from 'services/UserService'`
  - frontend内: `import { Button } from 'components/Button'`
  - 注意: `backend/src/` や `src/` から始めない（baseUrl相対）
