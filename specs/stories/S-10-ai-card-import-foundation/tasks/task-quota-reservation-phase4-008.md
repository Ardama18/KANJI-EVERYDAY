...
- [x] `IT-QUOTA-01〜08`をfixed DB clock/複数connectionの失敗テストへ置換する。
- [x] JST境界、199+1/199+2、49+1/49+2、異なるkey並行、同key再送、exempt、開始後非返却を検証する。
...
- [x] `P1000〜P1008`と`42501`を安定code/safe detailへ対応させ、不明な`23505`を一律duplicateにしない。
- [x] service_role専用wrapperと外部grantなしinternalを分離し、owner/source/hash/kind/units/関連を再検証する。
- [x] advisory→既存reservation non-lock read→batch/item→usage `FOR UPDATE`→reservationの順を固定する。
- [x] DB clockを`Asia/Tokyo`へ変換し、card 200/image 50超過だけを`QUOTA_EXCEEDED`にする。
- [x] 同key同内容は同じ結果/加算0、差分は`CONFLICT`、予約成功時に`provider_started_at`を設定し返却しない。

### 3. Refactor Phase

- [x] test-only clock/failpointを公開RPCと分離し、production clockをclient値で上書き不能にする。
- [x] error/logにgeneration hash、reservation key、本文、SQL、stackを出さない。

## 完了条件

- [x] `IT-QUOTA-01〜08`が実DBでPASSする。
- [x] 並行成功合計がcard 200/image 50を超えず、同key再送の二重消費が0である。
- [x] remote_mcp/uploadの免除をclient flagで偽装できない。
- [x] authenticatedからwrapper、service_roleからinternalを直接実行できない。
- [x] 動作確認レベルL2を満たす。
