...
- [ ] fixtureへ固定White_Space全点、U+FEFF、NFKC、case、BMP外、結合文字、U+001F、generation/import差分vectorを定義する。
- [ ] `UT-NORM-01〜05`をfixture駆動の失敗テストへ置換する。
- [ ] VitestがS-10のUnit/Integration/E2E 3ファイルを検出し、TODO inventoryが32/61/13であることを確認する。
...
- [ ] `normalizeDisplayText`はNFKC→固定White_SpaceのASCII SPACE化→trim/連続圧縮を行いcaseを保持する。
- [ ] `normalizeForKey`は表示正規化後だけUnicode lowercaseを適用し、U+FEFFを空白扱いしない。
- [ ] harnessは型付きquery/actor/snapshot/parallel connection/test clock/failpointを提供し、secretや本文をerrorへ含めない。
- [ ] `test:s10:unit|integration|e2e|fresh|upgrade|failure|inventory`を個別選択可能にし、DB commandはURL未指定時にfail-fastする。
- [ ] frozen SeedはS-10前のID・本文・skill・pattern・deck関連・旧keyを再現し、repoの更新Seedと独立させる。
...
- [ ] S-02 helperを破壊変更せず、S-10固有actor/job責務をS-10 helperへ閉じる。
- [ ] fixtureの期待値をテスト本文へ複製せず、`any`、外部network/provider/Storage起動がないことを確認する。

## 完了条件

- [ ] `UT-NORM-01〜05`がPASSし、残るUnit TODOは27件である。
- [ ] 固定White_Space全点とU+FEFF非空白、NFKC、case、BMP外/結合文字の結果がfixtureと一致する。
- [ ] Unit/Integration/Contract E2Eと3種類のDB jobを別commandで選択できる。
- [ ] DB URL未指定時のIntegration/jobがfallback接続せず安全に失敗する。
- [ ] `npm --prefix frontend run typecheck`がPASSする。
- [ ] 動作確認レベルL2（対象Unit + harness境界）が満たされる。
