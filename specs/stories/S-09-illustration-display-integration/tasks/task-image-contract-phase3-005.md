...
### 1. Red Phase
- [x] `IllustrationDisplay.test.tsx` に `<Image>` props契約の失敗テストを追加する。
- [x] `next.config.mjs` の remotePatterns 契約を静的に検証する失敗テストを追加する（既存テストファイルに統合可）。
...
### 2. Green Phase
- [x] `frontend/next.config.mjs` を新規作成し、Supabase署名URL向け `remotePatterns` を設定する。
- [x] `frontend/vitest.config.ts` を必要最小限更新し、story配下テスト実行経路を維持する。
- [x] `<Image>` props契約のテストを通過させる。
- [x] 統合テスト全件を再実行してPhase 1〜3の退行がないことを確認する。
...
### 3. Refactor Phase
- [x] 設定値・テスト名を契約用語（AC-18/19）へ統一する。
- [x] 統合テストを再実行して最終状態を固定する。
...
## 完了条件
- [x] AC-18/19 の静的契約がテストで追跡可能になっている。
- [x] `next.config.ts` を変更せず、`next.config.mjs` のみで要件を満たしている。
- [x] Phase 1〜3 の統合テスト全件がPASSしている。
...
## 作業ログ
- 2026-02-24: `quality-fixer` 手順のフォールバックとして `npm run check --prefix frontend` を実行。S-04 seed 系テストが `psql ... global/pg_filenode.map: I/O error`（`127.0.0.1:54322`）で失敗し、`status=approved` は未達。
