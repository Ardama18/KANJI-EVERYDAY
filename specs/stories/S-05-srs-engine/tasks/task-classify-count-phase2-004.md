...
### 1. Red Phase
- [ ] `classify.test.ts` に AC#12〜#15 の失敗テストを追加する。
- [ ] `srs-engine.int.test.ts` の `IT-AC12`〜`IT-AC15` を失敗テストへ変更する。
- [ ] 失敗を確認する。
...
### 2. Green Phase
- [ ] `classify.ts` に分類判定と集計ロジックを最小実装する。
- [ ] `null` 除外条件を明示して集計仕様を実装する。
- [ ] 対象テストがPASSするまで判定順序を調整する。
...
### 3. Refactor Phase
- [ ] 分類条件を読みやすい述語関数へ整理する（挙動不変）。
- [ ] 境界ケース（today一致/未一致）をテーブル駆動で明確化する。
- [ ] 対象テスト再実行で回帰がないことを確認する。
...
## 完了条件
- [ ] AC#12〜#15 が Unit + Integration でPASSしている。
- [ ] `classifyCard` / `countByCategory` が pure 関数として実装されている。
- [ ] 変更差分が Phase 2前半の論理単位として1コミット可能。
