...
### 1. Red Phase
- [ ] `prompt.test.ts` に sanitize の境界テスト（制御文字・長文）を追加する。
- [ ] `gemini-client.test.ts` に `fetch` 成功/失敗分類の失敗テストを追加する。
- [ ] 失敗を確認する。
...
### 2. Green Phase
- [ ] `prompt.ts` に sanitize + prompt 生成を実装する。
- [ ] `types.ts` に model_infoの成功/失敗契約型を追加する。
- [ ] `gemini-client.ts` に `fetch` 実装と失敗分類ロジックを実装する。
- [ ] 追加したunitテストを通す。
...
### 3. Refactor Phase
- [ ] Geminiレスポンス変換の重複処理をヘルパーへ整理する。
- [ ] 型ガードを追加して `any` 依存を排除する。
- [ ] unitテストを再実行する。
...
## 完了条件
- [ ] AC-14（制御文字除去 + 100文字上限）を満たす。
- [ ] AC-09（Gemini `fetch` 実装、SDK依存追加なし）を満たす。
- [ ] model_info へ失敗理由を格納する型契約が後続タスクで利用可能になっている。
- [ ] `prompt.test.ts` と `gemini-client.test.ts` がPASSしている。
- [ ] 動作確認レベル L2（対象Unit）が満たされている。
