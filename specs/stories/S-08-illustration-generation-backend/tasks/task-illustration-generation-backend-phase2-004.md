...
## 対象ファイル
- [x] `frontend/src/lib/illustration/storage.ts`
- [x] `frontend/src/lib/illustration/generator.ts`
- [x] `frontend/src/lib/illustration/generator.test.ts`
...
### 1. Red Phase
- [x] `generator.test.ts` に成功系/失敗系/APIキー未設定ケースの失敗テストを追加する。
- [x] `illustration-generation-backend.int.test.ts` の `IT-AC02/08/09/10/11/14` を失敗テストへ変更する。
- [x] 失敗を確認する。
...
### 2. Green Phase
- [x] `storage.ts` で `{user_id}/{illustration_id}.png` パス生成とアップロード処理を実装する。
- [x] `generator.ts` で prompt -> Gemini -> Storage -> DB更新の処理を実装する。
- [x] APIキー未設定時の fail-safe と `model_info.reason` 記録を実装する。
- [x] Unit/IntegrationのPhase 2対象テストを通す。
...
### 3. Refactor Phase
- [x] generator内の例外ハンドリングとreasonマッピングを整理する。
- [x] Storage path builder を単一責務に保ち、再利用点を明確化する。
- [x] Phase 2対象テストを再実行して回帰がないことを確認する。
...
## 完了条件
- [x] AC-02/08/10/11 を満たす実装とテストが揃っている。
- [x] Phase 2対象統合テスト `IT-AC02/08/09/10/11/14` がPASSしている。
- [x] APIキー未設定時に外部API呼び出し0回で `failed` 収束する。
- [x] Storage key が `{user_id}/{illustration_id}.png` で固定されている。
- [x] Phase 2 完了チェック（対象AC + 対象統合テストPASS）が本タスク内で完結している。
- [x] 動作確認レベル L2（対象Unit + Integration）が満たされている。

## 動作確認
- [x] `npm run test --prefix frontend -- src/lib/illustration/generator.test.ts`
- [x] `npm run test --prefix frontend -- ../specs/stories/S-08-illustration-generation-backend/tests/illustration-generation-backend.int.test.ts -t "IT-AC02|IT-AC08|IT-AC09|IT-AC10|IT-AC11|IT-AC14"`
