...
### 1. Red Phase
- [ ] `seed-data-and-utilities.int.test.ts` の `IT-AC07`〜`IT-AC09` を `it.todo` から失敗テストへ切り替える。
- [ ] `specs/stories/S-02-database-schema-rls/tests/helpers/s02-db-testkit.ts` の `auth.users` 必須列セットを確認する。
- [ ] Seed 未実装状態で失敗を確認し、必要な SQL 契約（fixed UUID、必須列、カード件数）を確定する。
...
### 2. Green Phase
- [ ] `supabase/seed.sql` に 50字ソース定義を実装し、各行から R1/W1 cards を生成する。
- [ ] `auth.users` を必須列セットで `ON CONFLICT (id)` upsert し、`users_profile` を同期 upsert する。
- [ ] `decks` を `SEED_DECK_ID` 固定で upsert し、`name='小学3年生の漢字'` と `new_limit_per_day=10` を保証する。
...
### 3. Refactor Phase
- [ ] Seed SQL の CTE 構成を整理し、cards 生成と owner/deck upsert の責務を明確化する。
- [ ] `card_key` 生成式を一箇所に固定し、契約逸脱を防ぐ。
...
## 完了条件
- [ ] `supabase/seed.sql` に固定 UUID の owner/profile/deck upsert が実装されている（AC-09）。
- [ ] Seed 実行後に cards が 100件で、R1/W1 と card_key 契約を満たす（AC-07, AC-08）。
