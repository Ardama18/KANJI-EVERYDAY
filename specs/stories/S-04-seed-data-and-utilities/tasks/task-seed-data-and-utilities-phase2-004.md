...
### 1. Red Phase
- [x] `seed-data-and-utilities.int.test.ts` の `IT-AC11`〜`IT-AC12` を `it.todo` から失敗テストへ切り替える。
- [x] Seed 再実行で件数増加または失敗時部分反映が起こることを失敗で観測する。
...
### 2. Green Phase
- [x] `supabase/seed.sql` を `BEGIN` 開始・全処理成功時 `COMMIT` の単一トランザクションにする。
- [x] `cards` は `ON CONFLICT (card_key) DO NOTHING`、`decks` は `ON CONFLICT (id) DO UPDATE`、`deck_cards` は `ON CONFLICT (deck_id, card_id) DO NOTHING` を適用する。
- [x] `IT-AC11`〜`IT-AC12` を pass させ、続けて `IT-AC01`〜`IT-AC12` 全件で回帰なしを確認する。
...
### 3. Refactor Phase
- [x] トランザクション境界と失敗注入ケースの可読性を改善し、テスト意図を明確化する。
- [x] Phase 2 対象テストを再実行して回帰がないことを確認する。
...
## 完了条件
- [x] Seed 再実行後に `cards/decks/deck_cards` 件数が増えない（AC-11）。
- [x] 失敗時に明示的エラーが返り、部分成功データが残らない（AC-12）。
- [x] `IT-AC11`〜`IT-AC12` が pass し、`IT-AC01`〜`IT-AC10` へ回帰がない。
- [x] 動作確認レベル L2（対象 Integration）が満たされている。

## 動作確認
- [ ] `supabase db reset`
- [x] `npm --prefix frontend run test -- ../specs/stories/S-04-seed-data-and-utilities/tests/seed-data-and-utilities.int.test.ts -t "IT-AC11|IT-AC12"`
...
注記: `supabase` CLI は不在のため `supabase db reset` は未実行。Integration は `S02_TEST_DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:54322/postgres` 指定で `IT-AC11|IT-AC12` と `IT-AC01|...|IT-AC06` / `IT-AC07|...|IT-AC12` を分割実行して確認した。
