...
- Integration suite: `specs/stories/S-11-ai-card-async-processing/tests/ai-card-async-processing.int.test.ts`（現行223件。127/139/142/147/153/154/155/162/163/168/179/181/189/195/198/200/216/217件は履歴値）
...
- [x] R21-F1 Red/Green: service-only snapshotのAuthorization Bearerとapikeyをcase-insensitiveに検査し、同一credential pairだけを許可する。
- [x] R21-F1: 明示headerを先に`Headers`へ正規化し、apikeyが存在しない場合だけanon defaultを補完することで、大文字`APIKEY`を含むexplicit service apikeyを保持する。
- [x] R21-F1: canonical pair、AUTHORIZATION/APIKEY case variant、Authorization欠落、apikey欠落、owner bearer+anon spoof、service credential mismatchの6ケースを表駆動で検査し、拒否時fetch 0を固定する。
- [x] R21-F3: ordinary Vitestの別file workerでS-10 integration/E2Eが共有usage ownerを同時使用し、5秒timeout後の一時状態を相互観測する非決定性を、ordinary phaseだけ`maxWorkers=1/minWorkers=1`へ固定して除去する。各file内部の並行commit/lock検査は維持する。
