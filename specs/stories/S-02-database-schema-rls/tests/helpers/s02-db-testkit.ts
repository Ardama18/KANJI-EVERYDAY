...
function psql(args: string[]): string {
	const baseArgs = [getDatabaseUrl(), "-v", "ON_ERROR_STOP=1", "-X", "-A", "-t", "-q"];
...
export function buildOwnerScopedPath(ownerUserId: string, leafName: string): string {
	return `${ownerUserId}/${leafName}`;
}
