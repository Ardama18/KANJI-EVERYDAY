...
argument-hint: '<要件説明 or ストーリーディレクトリパス> [追加指示]'
...
または
```
/infrastructure <ストーリーディレクトリパス> [追加指示]
```

**引数**：
- `<要件説明>` または `<ストーリーディレクトリパス>`: インフラ要件の説明またはストーリーディレクトリパス（例: `specs/stories/TC-S-34-xxx`）
...
| ストーリーディレクトリパス付き | ディレクトリパスが指定されている | ローカルの story.md / requirements.md / meta.json をRead→ストーリー情報取得→infrastructure-designerから開始 |
...
### 例4: ストーリーディレクトリからのインフラTask
```
/infrastructure specs/stories/TC-S-34-security-enhancement セキュリティ強化を優先
```
