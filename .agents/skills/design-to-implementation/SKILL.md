...
argument-hint: '<ストーリーディレクトリパス> [追加指示]'
...
```
/design-to-implementation <ストーリーディレクトリパス> [追加指示]
```

**引数**：
- `<ストーリーディレクトリパス>` (必須): ストーリーディレクトリパス（例: `specs/stories/TC-S-34-database-schema-prisma`）
...
### 前提となるコマンド
通常、以下のコマンドの後に実行します：
- `/story-to-design` - ストーリーから設計・計画まで実施済み
...
✅ PR作成・meta.json更新（ステータス更新、PR情報記録）
...
### 使用シーン
- `/story-to-design` で設計レビューを受けた後、実装を開始したい
...
いずれかが欠けている場合：
- エラーメッセージを表示
- `/story-to-design <ストーリーディレクトリパス>` の実行を促す
- 処理を中断
...
4. ✅ PRが作成されている
5. ✅ meta.jsonのstatusが「implementation_review」に更新されている
6. ✅ meta.jsonにPR情報が記録されている
...
4. meta.jsonのstatusを"implementation_review"に更新（Editツール使用）
...
2. **進捗管理**
   - **task-executorがタスクファイルのチェックボックスを更新**（`[ ]` → `[x]`）
   - オーケストレーターはTodoWriteで全体進捗を管理
   - 1タスク完了ごとに両方のステータス更新を確認
...
※ quality-fixer、ui-fixer、visual-checkerはtask-executorが内部で実行済み

### Phase 4: PR作成・meta.json更新
1. **PR作成**
   - gh pr create でPR作成
   - タイトル: ストーリータイトル
   - 本文: 実装内容、テスト結果、変更ファイル数

2. **meta.jsonのstatusを"implementation_review"に更新**（Editツール使用）
...
- [ ] PRが作成されている
- [ ] meta.jsonのstatusが"implementation_review"に更新されている
...
- 対処: エラーメッセージを表示、`/story-to-design` の実行を促す
...
- 必要に応じて `/story-to-design` から再実行

## 使用例

```bash
# 例1: ストーリーディレクトリパスで指定
/design-to-implementation specs/stories/TC-S-34-database-schema-prisma

# 例2: 追加指示付き
/design-to-implementation specs/stories/TC-S-35-parent-auth-system "パフォーマンス重視で実装"
```
...
- タスク管理と品質保証が自動化される
- meta.json更新が自動化され、進捗管理が容易

**前提条件**：
- `/story-to-design` で設計フェーズが完了していること
- ストーリーディレクトリ内に requirements.md、ADR、design.md、plan.md が存在すること
