...
**責務外**（SKILL.mdが実施）:
- ストーリーディレクトリ作成（`specs/stories/`） - ストーリーID採番後に作成
- 各ストーリーの`meta.json`生成 - ストーリーID確定後に作成
...
- **親ストーリー情報**
  - storyId（親ストーリーを一意に識別する識別子、例: TC-S-34）
  - title（ケバブケース形式）
  - description（概要）
  - requirements（要件）
  - epicId（所属エピックID）
...
**注意**: ストーリーIDは含めません。SKILL.mdがspecs/stories/ディレクトリを走査して採番するため、分解ロジックでは決定しません。
...
**重要な変更点**:
- ストーリーディレクトリは作成**しない**
- ストーリーディレクトリはSKILL.mdがストーリーID採番後に作成
- 理由: ストーリーIDはSKILL.mdがspecs/stories/を走査して採番するため、分解ロジックでは確定しない
...
**理由**: 管理情報はmeta.jsonで管理、技術情報のみepic.md/design.mdで管理（Single Source of Truth原則）。詳細は .claude/steering/story-structure.md 参照。
...
```json
{
  ...
  "data": {
    "epicId": "TC-E-1",
    "epicDirectory": "specs/epics/TC-E-1-authentication-system/",
    "stories": [
      {
...
**重要な変更点**:
- `storyId`は含まれません（SKILL.mdがspecs/stories/を走査して採番するため）
- `directory`は含まれません（ディレクトリはID採番後に作成されるため）
